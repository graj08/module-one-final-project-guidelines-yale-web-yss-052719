...
def areyouHungry(activeUser)
    ...
    x = Recipe.all.select do |recipe|
        #binding.pry
        recipe.name.downcase == userHunger.downcase || recipe.recipe_ingredients.map  {|x| x.ingredient}.map {|x| x.name}.any? {|x| x.downcase == userHunger.downcase} || recipe.name.split(" "). any? { |x| x.downcase == userHunger.downcase }
    end 
    if x.empty?
...
def doRecipe(activeUser)
    ...
    system("clear")
    puts "Ok, we are adding #{userRecipe.name} to your recipe list now."
    puts "Here are the instructions for your recipe:"
    userRecipe.process.split(".").each { |x| puts x }
    puts "How many portions of that recipe would you like to make?"
...
def extraOptions(activeUser)
    system("clear")
    userInput  = ""
    until userInput.downcase == "Done".downcase
        puts "Is there anything else that you would like to do?"
        puts "Options: Create another recipe, view leftovers, view shopping list, buy groceries."
        puts "If you are done using our application, please type 'Done' and press enter"
        userInput = gets.chomp 
        if userInput.downcase == "Create another recipe".downcase
            system("clear")
            ...
            x = Recipe.find_by(name: userInput3[0], process: userInput3[1], servings: userInput3[2])
            userInput2[3..999].each { |x| Ingredient.create(name: x)}
            userInput2[3..999].each { |x| RecipeIngredient.create(ingredient: Ingredient.find_by(name: x),recipe: x2) } 
            puts "Ok. Your recipe has been added!"
        elsif userInput.downcase == "View leftovers".downcase
            system("clear")
            activeUser.leftovers.each do |x, y|
            puts "#{x.name}, #{y.round(2)}"
        end
        elsif userInput.downcase == "View Shopping List".downcase
            system("clear")
            activeUser.leftovers.each do |x, y|
            puts "#{x.name}, #{y.round(2)}"
        end
        elsif userInput.downcase == "Buy Groceries".downcase 
            system("clear")
            ...
            userInput = gets.chomp.split(",")
            userInput.each { |x| activeUser.buy_ingredient(Ingredient.find_or_create_by(name: x))}
